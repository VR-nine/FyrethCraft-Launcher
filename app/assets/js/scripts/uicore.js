        ...
        switch(arg){
            ...
            case 'update-available':
                ...
                if(process.platform === 'darwin'){
                    info.darwindownload = `https://github.com/VR-nine/FyrethCraft-Launcher/releases/download/v${info.version}/FyrethCraft-Launcher-setup-${info.version}${process.arch === 'arm64' ? '-arm64' : '-x64'}.dmg`
                    showUpdateUI(info)
                }
...
function showUpdateUI(info){
    ...
    if (!sealContainer) {
        loggerAutoUpdater.warn('image_seal_container element not found, cannot show update UI')
        return
    }
    sealContainer.setAttribute('update', true)
    sealContainer.onclick = () => {
